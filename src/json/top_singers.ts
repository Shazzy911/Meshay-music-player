import { TopSingersType } from "@/types/top_singers.types";

export const release: TopSingersType[] = [
  {
    id: 1,
    newImage:
      "https://i.pinimg.com/236x/ae/f1/62/aef162270cda91d4c546b531dbf9a589.jpg", // Add your image URL
    mainTitle: "Taylor Swift",
    subTitle: "Artist",
    songs: [
      {
        id: 1,
        title: "Anti-Hero",
        featuring: "Taylor Swift",
        time: "3:20",
        album: "Midnights",
        image:
          "https://i.pinimg.com/236x/c0/95/f3/c095f3ecad6eedb2eae670475e0a1b92.jpg", // Add your image URL
      },
      {
        id: 2,
        title: "Blank Space",
        featuring: "None",
        time: "3:51",
        album: "1989",
        image:
          "https://i.pinimg.com/236x/c0/95/f3/c095f3ecad6eedb2eae670475e0a1b92.jpg", // Add your image URL
      },
      {
        id: 3,
        title: "All Too Well (10 Minute Version)",
        featuring: "Taylor Swift",
        time: "10:13",
        album: "Red (Taylor's Version)",
        image:
          "https://i.pinimg.com/236x/c0/95/f3/c095f3ecad6eedb2eae670475e0a1b92.jpg", // Add your image URL
      },
      {
        id: 4,
        title: "Lover",
        featuring: "Taylor Swift",
        time: "3:41",
        album: "Lover",
        image:
          "https://i.pinimg.com/236x/c0/95/f3/c095f3ecad6eedb2eae670475e0a1b92.jpg", // Add your image URL
      },
      {
        id: 5,
        title: "You Belong with Me",
        featuring: "Taylor Swift",
        time: "3:52",
        album: "Fearless",
        image:
          "https://i.pinimg.com/236x/c0/95/f3/c095f3ecad6eedb2eae670475e0a1b92.jpg", // Add your image URL
      },
    ],
  },
  {
    id: 2,
    newImage:
      "https://i.pinimg.com/236x/4b/7b/c9/4b7bc96c63cb960f1ae5eaac08cec28f.jpg", // Add your image URL
    mainTitle: "Ed Sheeran",
    subTitle: "Artist",
    songs: [
      {
        id: 1,
        title: "Shape of You",
        featuring: "Ed Sheeran",
        time: "4:24",
        album: "Divide",
        image:
          "https://i.pinimg.com/236x/d0/41/74/d04174156447963eb85b76a9e1684a98.jpg", // Add your image URL
      },
      {
        id: 2,
        title: "Perfect",
        featuring: "Ed Sheeran",
        time: "4:23",
        album: "Divide",
        image:
          "https://i.pinimg.com/236x/d0/41/74/d04174156447963eb85b76a9e1684a98.jpg", // Add your image URL
      },
      {
        id: 3,
        title: "Bad Habits",
        featuring: "Ed Sheeran",
        time: "3:50",
        album: "Equals",
        image:
          "https://i.pinimg.com/236x/d0/41/74/d04174156447963eb85b76a9e1684a98.jpg", // Add your image URL
      },
      {
        id: 4,
        title: "Thinking Out Loud",
        featuring: "Ed Sheeran",
        time: "4:41",
        album: "Multiply",
        image:
          "https://i.pinimg.com/236x/d0/41/74/d04174156447963eb85b76a9e1684a98.jpg", // Add your image URL
      },
      {
        id: 5,
        title: "I Don't Care",
        featuring: "Ed Sheeran, Justin Bieber",
        time: "3:39",
        album: "No.6 Collaborations Project",
        image:
          "https://i.pinimg.com/236x/d0/41/74/d04174156447963eb85b76a9e1684a98.jpg", // Add your image URL
      },
    ],
  },
  {
    id: 3,
    newImage:
      "https://i.pinimg.com/236x/b9/7d/bb/b97dbb0c48f4580202cb02b2d75b3127.jpg", // Add your image URL
    mainTitle: "Adele",
    subTitle: "Artist",
    songs: [
      {
        id: 1,
        title: "Hello",
        featuring: "Adele",
        time: "4:55",
        album: "25",
        image:
          "https://i.pinimg.com/236x/5e/65/ae/5e65ae0d654d5768b213cab634ea327c.jpg", // Add your image URL
      },
      {
        id: 2,
        title: "Someone Like You",
        featuring: "Adele",
        time: "4:45",
        album: "21",
        image:
          "https://i.pinimg.com/236x/5e/65/ae/5e65ae0d654d5768b213cab634ea327c.jpg", // Add your image URL
      },
      {
        id: 3,
        title: "Easy On Me",
        featuring: "Adele",
        time: "3:44",
        album: "30",
        image:
          "https://i.pinimg.com/236x/5e/65/ae/5e65ae0d654d5768b213cab634ea327c.jpg", // Add your image URL
      },
      {
        id: 4,
        title: "Rolling in the Deep",
        featuring: "Adele",
        time: "3:48",
        album: "21",
        image:
          "https://i.pinimg.com/236x/5e/65/ae/5e65ae0d654d5768b213cab634ea327c.jpg", // Add your image URL
      },
      {
        id: 5,
        title: "Set Fire to the Rain",
        featuring: "Adele",
        time: "4:02",
        album: "21",
        image:
          "https://i.pinimg.com/236x/5e/65/ae/5e65ae0d654d5768b213cab634ea327c.jpg", // Add your image URL
      },
    ],
  },
  {
    id: 4,
    newImage:
      "https://i.pinimg.com/736x/af/5c/b5/af5cb5e17604dfb42b89e8fe522bc596.jpg", // Add your image URL
    mainTitle: "Lana Del Rey",
    subTitle: "Artist",
    songs: [
      {
        id: 1,
        title: "Young and Beautiful",
        featuring: "Lana Del Rey",
        time: "3:56",
        album: "The Great Gatsby Soundtrack",
        image:
          "https://i.pinimg.com/236x/6f/e6/aa/6fe6aa6a443c3cb09037f46e8d86615c.jpg", // Add your image URL
      },
      {
        id: 2,
        title: "Summertime Sadness",
        featuring: "Lana Del Rey",
        time: "4:25",
        album: "Born to Die",
        image:
          "https://i.pinimg.com/236x/6f/e6/aa/6fe6aa6a443c3cb09037f46e8d86615c.jpg", // Add your image URL
      },
      {
        id: 3,
        title: "West Coast",
        featuring: "Lana Del Rey",
        time: "4:16",
        album: "Ultraviolence",
        image:
          "https://i.pinimg.com/236x/6f/e6/aa/6fe6aa6a443c3cb09037f46e8d86615c.jpg", // Add your image URL
      },
      {
        id: 4,
        title: "Don't Call Me Angel",
        featuring: "Lana Del Rey, Ariana Grande, Miley Cyrus",
        time: "3:10",
        album: "Charlie's Angels Soundtrack",
        image:
          "https://i.pinimg.com/236x/6f/e6/aa/6fe6aa6a443c3cb09037f46e8d86615c.jpg", // Add your image URL
      },
      {
        id: 5,
        title: "Video Games",
        featuring: "Lana Del Rey",
        time: "4:42",
        album: "Born to Die",
        image:
          "https://i.pinimg.com/236x/6f/e6/aa/6fe6aa6a443c3cb09037f46e8d86615c.jpg", // Add your image URL
      },
    ],
  },
  {
    id: 5,
    newImage:
      "https://i.pinimg.com/474x/6c/e9/5e/6ce95ed6612dca88aa318b87ce3c0eb0.jpg", // Add your image URL
    mainTitle: "Billie Eilish",
    subTitle: "Artist",
    songs: [
      {
        id: 1,
        title: "Bad Guy",
        featuring: "Billie Eilish",
        time: "3:14",
        album: "When We All Fall Asleep, Where Do We Go?",
        image:
          "https://i.pinimg.com/236x/49/66/3d/49663d8696a9e09213f4d5384d28bfab.jpg", // Add your image URL
      },
      {
        id: 2,
        title: "Ocean Eyes",
        featuring: "Billie Eilish",
        time: "3:20",
        album: "Don't Smile at Me",
        image:
          "https://i.pinimg.com/236x/49/66/3d/49663d8696a9e09213f4d5384d28bfab.jpg", // Add your image URL
      },
      {
        id: 3,
        title: "Everything I Wanted",
        featuring: "Billie Eilish",
        time: "4:05",
        album: "Single",
        image:
          "https://i.pinimg.com/236x/49/66/3d/49663d8696a9e09213f4d5384d28bfab.jpg", // Add your image URL
      },
      {
        id: 4,
        title: "Lovely",
        featuring: "Billie Eilish, Khalid",
        time: "3:20",
        album: "Single",
        image:
          "https://i.pinimg.com/236x/49/66/3d/49663d8696a9e09213f4d5384d28bfab.jpg", // Add your image URL
      },
      {
        id: 5,
        title: "Happier Than Ever",
        featuring: "Billie Eilish",
        time: "4:58",
        album: "Happier Than Ever",
        image:
          "https://i.pinimg.com/236x/49/66/3d/49663d8696a9e09213f4d5384d28bfab.jpg", // Add your image URL
      },
    ],
  },
];
